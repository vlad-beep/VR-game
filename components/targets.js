    ...
    function createFormation() {
      ...
      if (currentTargetCount < maxTargets) {
        ...
        for (let i = 0; i < targetsPerFormation; i++) {
          ...
          target.setAttribute('id', 'blueTarget' + currentTargetCount + i);
...
AFRAME.registerComponent('spawn-red-targets', {
  init: function () {
    const scene = this.el;
    const maxTargets = 30;
    const spacing = 2;
    ...
    function createFormation() {
      ...
      if (currentTargetCount < maxTargets) {
        ...
        for (let i = 0; i < targetsPerFormation - 1; i++) {
          const target = document.createElement('a-entity');
          target.setAttribute('gltf-model', '#blueTarget');
          target.setAttribute('scale', '10 10 10');
          target.setAttribute(
        ...
        const target = document.createElement('a-entity');
        target.setAttribute('gltf-model', '#redTarget');
        target.setAttribute('scale', '10 10 10');
        target.setAttribute(
          'animation',
          'property: rotation; to: 0 360 0; dur: 10000; easing: linear; loop: true;',
        ...
        if (formationType === 0) {
          target.setAttribute('position', {
            x: randomX + (targetsPerFormation - 1) * spacing - (targetsPerFormation / 2) * spacing,
            y: randomY,
            z: -15,
          });
        } else {
          target.setAttribute('position', {
            x: randomX,
            y: randomY + (targetsPerFormation - 1) * spacing - (targetsPerFormation / 2) * spacing,
            z: -15,
          });
        }

        target.setAttribute('id', 'redTarget' + currentTargetCount + (targetsPerFormation - 1));
        target.setAttribute('data-raycastable', '');
        scene.appendChild(target);
      }
    }
    createFormation();
    redTargetSpawnInterval = setInterval(createFormation, 3000);
  },
});
